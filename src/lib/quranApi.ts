...
/** Startseiten der 30 Juz (aus juz-starts.json). */
...
/** Liefert Start- und Endseite eines Juz (synchron, für Anzeige). */
export function getJuzPageRange(juzNumber: number): { start: number; end: number } {
  const start = JUZ_START_PAGES[juzNumber] ?? 1;
  const end = juzNumber < 30 ? (JUZ_START_PAGES[juzNumber + 1] ?? 604) - 1 : 604;
  return { start, end };
}
